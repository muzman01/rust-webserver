...
impl RustaceanRepository {
    ...
    pub async fn update(c: &mut AsyncPgConnection, id: i32, rustaceans:Rustacean) -> QueryResult<Rustacean> {
    ...
    pub async fn create(c: &mut AsyncPgConnection, new_user: NewUser, role_codes: Vec<String>) -> QueryResult<User> {
       let user = diesel::insert_into(users::table)
        ...
        for role_code in role_codes {
            let new_user_role = {
               if let Ok(role) = RoleRepository::find_by_code_role(c, role_code.to_owned()).await{
                NewUserRole {
                    user_id: user.id,
                    role_id: role.id
                }
...
impl RoleRepository {
    ...
    pub async fn find_by_code_role(c: &mut AsyncPgConnection,role_code: String) -> QueryResult<Role> {
